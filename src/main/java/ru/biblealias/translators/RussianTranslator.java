...
public class RussianTranslator {
    ...
    public Command getBotsCommand(String message) {
        if(message.toLowerCase(Locale.ROOT).contains("привет")) {
            return Command.HELLO;
        }
        else if (message.toLowerCase(Locale.ROOT).contains("отзыв:")) {
            return Command.FEEDBACK_MESSAGE;
        }
        else {
            return switch (message) {
                case "Да", "да" -> Command.YES;
                case "Нет", "нет" -> Command.NO;
                case "Играть, используя текущие настройки" -> Command.PLAY_USING_THE_CURRENT_SETTINGS;
                case "Начать новую игру" -> Command.START_NEW_GAME;
                case "Завершить игру" -> Command.STOP_GAME;
                case "Настройки" -> Command.SETTINGS;
                case "Инструкция" -> Command.INSTRUCTION;
                case "Оставить отзыв" -> Command.SEND_FEEDBACK;
                case "Изменить настройки" -> Command.CHANGE_SETTINGS;
                case "Язык" -> Command.LANGUAGE;
                case "Русский язык" -> Command.RUSSIAN;
                case "English language" -> Command.ENGLISH;
                case "Українська мова" -> Command.UKRAINIAN;
                case "Режим игры" -> Command.GAME_MODE;
                case "Ветхий Завет \uD83D\uDD4E" -> Command.OLD_TESTAMENT;
                case "Новый Завет ✝️" -> Command.NEW_TESTAMENT;
                case "Вся Библия \uD83D\uDCD6" -> Command.ALL_BIBLE;
                case "Рождественский alias ✨" -> Command.CHRISTMAS;
                case "Длительность раунда" -> Command.ROUND_DURATION;
                case "Штраф за неотгаданное слово" -> Command.PENALTY;
                case "1 очко" -> Command.PENALTY_ONE;
                case "2 очка" -> Command.PENALTY_TWO;
                case "Без штрафа" -> Command.PENALTY_ZERO;
                case "Получить задание \uD83D\uDCAC" -> Command.GET_TASK;
                case "Начать отгадывать слова" -> Command.START_GUESSING_WORDS;
                case "Отгадали ✔️" -> Command.GUESSED;
                case "Не отгадали ❌" -> Command.NOT_GUESSED;
    ...
    public String translateAnswersMessage(AnswersMessage message) {
        return switch (message) {
            ...
            case SUCCESSFUL_AUTHORIZATION -> "Ваша авторизация прошла успешно! \uD83D\uDC4D" +
                    "\nТеперь Вы можете играть в \"Библейский alias\" с помощью этого бота. " +
                    "\nВнизу слева есть меню. Выберите там \"Начать новую игру\" и играйте, следуя указаниям бота \uD83E\uDD16 " +
                    "Для завершения игры выберите в меню \"Завершить игру\" или напишите боту \"Завершить игру\"";
            ...
            case WELCOME_MESSAGE -> """
                ...
                Вы можете сыграть 1 пробную игру - для этого нажвите в меню "Начать новую игру".
                Для получения полного доступа к боту пройдите по ссылке в наш магазин и оформите подписку:
                https://biblealias.ru/products/Bibleysky-alias-Bot-p406365873
                Если у Вас уже есть код авторизации (номер Вашего заказа в формате #*****), отправьте его в сообщении.
                """;
